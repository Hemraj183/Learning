...
function initNavigation() {
    ...
    navLinks.forEach(link => {
        link.addEventListener('click', (e) => {
            ...
            // External/relative file links (like Main Menu) will follow their default behavior
        });
    });
}
...
function initPythonTerminal() {
    ...
    const exampleSelect = document.getElementById('example-snippets');
    ...
    exampleSelect.addEventListener('change', (e) => {
        ...
        if (example) {
            codeInput.value = getExampleCode(example);
            exampleSelect.value = '';
        }
    });
    ...
    async function loadPyodide() {
        pyodideLoading = true;
        terminalStatus.textContent = 'Loading Python...';
        ...
        try {
            ...
            terminalStatus.classList.add('ready');
            runBtn.disabled = false;

            addOutput('Python 3.11 (Pyodide) ready! NumPy loaded.', 'success');
            addOutput('Note: For PyTorch training, use local setup or Google Colab.', 'info');
        } catch (error) {
            terminalStatus.textContent = 'Error';
            terminalStatus.classList.add('error');
            addOutput(`Failed to load Python: ${error.message}`, 'error');
        } finally {
    ...
    async function runPythonCode() {
        if (!pyodide) {
            addOutput('Python is still loading... Please wait.', 'error');
            return;
        }
...
sys.stderr = StringIO()
            ...
            runBtn.disabled = false;
            runBtn.innerHTML = '<span class="btn-icon">▶</span> Run Code (Ctrl+Enter)';
        }
    }
    ...
    function addOutput(text, type = 'normal') {
        ...
        line.className = type === 'error' ? 'output-error' :
            type === 'success' ? 'output-success' :
                type === 'info' ? 'output-line' : 'output-line';
...
# NumPy operations (faster than loops!)
import numpy as np
arr = np.arange(1, 6)
print("\\nNumPy array:", arr)
print("Squared:", arr ** 2)`
...
// Wrapper for Pyodide load function
async function loadPyodideModule(config) {
    if (typeof loadPyodide !== 'undefined') {
        return await loadPyodide(config);
    } else {
        throw new Error('Pyodide not loaded. Please check your internet connection.');
    }
}
